/// - See: https://github.com/SwiftGen/SwiftGenPlugin
...
struct SwiftGenPlugin: CommandPlugin {
    func performCommand(context: PluginContext, arguments: [String]) async throws {
        ...
        let configFile = ".swiftgen.yml"
        let fileManager = FileManager.default

        let configuration = context.package.directory.appending(configFile)
        ...
        for target in targets {
            let configuration = target.directory.appending(configFile)
            if fileManager.fileExists(atPath: configuration.string) {
